...
# Load the Planetary Computer catalog
catalog = Client.open("https://planetarycomputer.microsoft.com/api/stac/v1")

# Define the bounding box around Mount St. Helens
bbox = (-122.259265, 46.149409, -122.112323, 46.250639)

# Search the catalog for Copernicus GLO-30 elevation data over Mount St. Helens
search = catalog.search(
    collections=["cop-dem-glo-30"], 
    bbox=bbox,
)

# Sign the search results with Planetary Computer to allow asset access
items = pc.sign(search)

# Download the data, squeeze out extra dimensions, and grab the Numpy array
dem = stackstac.stack(items, bounds_latlon=bbox).squeeze().values
dem.shape
```
...
![png](/images/terrain_from_scratch_files/terrain_from_scratch_5_1.png)
...
plt.imshow(w, cmap="terrain")
for (y, x), z in np.ndenumerate(w):
    plt.text(x, y, z, ha="center", va="center")
```


    
![png](/images/terrain_from_scratch_files/terrain_from_scratch_8_0.png)
...
![png](/images/terrain_from_scratch_files/terrain_from_scratch_33_1.png)
...
![png](/images/terrain_from_scratch_files/terrain_from_scratch_37_1.png)
...
![png](/images/terrain_from_scratch_files/terrain_from_scratch_40_1.png)
...
axs[0, 0].imshow(dem, cmap="terrain")

axs[0, 1].set_title("Hillshade")
axs[0, 1].imshow(hs, cmap="Greys_r")

axs[1, 0].set_title("Slope")
axs[1, 0].imshow(slope, cmap="RdBu_r")

axs[1, 1].set_title("Aspect")
axs[1, 1].imshow(aspect, cmap="magma")
```
...
![png](/images/terrain_from_scratch_files/terrain_from_scratch_42_1.png)
